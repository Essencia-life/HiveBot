...
bot.on('inline_query', async ctx => {
    ...
    const myBookigns = await coWorkingCalendar.getEvents([`telegramUserId=${ctx.from.id}`], new Date());
...
function createInlineQueryArticlesFromBookings(bookings: CalendarEvent[]) {
    return bookings.map(booking => {
        ...
        const title = startDate.toLocaleString('en-gb', {
            dateStyle: 'short',
            timeStyle: 'short',
            timeZone: 'Europe/Lisbon'
        }).replaceAll('/', '.') + ' - ' + endDate.toLocaleTimeString('en-gb', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: false,
            timeZone: 'Europe/Lisbon'
